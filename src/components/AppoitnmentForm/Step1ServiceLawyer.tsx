  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="mb-6">
          ...
          {services.map(s => (
            <label key={s.id} className="inline-flex items-center space-x-2">
              <input
                type="checkbox"
                value={String(s.id)}
                checked={Array.isArray(formData.service) && formData.service.includes(String(s.id))}
        ...
        {/* Deposit notice when 2 or more services selected */}
        {Array.isArray(formData.service) && formData.service.length >= 2 && (
          <p className="mt-2 text-sm text-orange-600">
            Từ 2 dịch vụ trở lên thì hệ thống xin phép thanh toán cọc trước tiền
          </p>
        )}
      </div>
      <div className="mb-4">
